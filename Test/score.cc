...
#if __x86_64__
const string defaultStanderBasic = "../Demo/lab2_demo_x64.exe";
#else
const string defaultStanderBasic = "../Demo/lab2_demo_x86.exe";
#endif
...
void clearTempFiles() {
    int r = system(
            "powershell /c \"if (Test-Path ./test_out) {rm ./test_out "
            "-Force}; if (Test-Path ./test_ans) {rm ./test_ans "
            "-Force}\"");
    (void) r;
}
...
int testTrace(const char *trace) {
    clearTempFiles();
    if (system((string() + "powershell /c \"cat " + trace + " | " +
                standerBasic + " > ./test_ans\"")
                       .c_str()) != 0) {
        return 1;
    }
    
    if (system((string() + "powershell /c \"cat " + trace + " | " + studentBasic + " > ./test_out\"").c_str()) != 0)
        return 2;
    if (system("powershell /c \"diff ./test_ans ./test_out\"")) return 4;
    // clearTempFiles();
    return 0;
}
    ...
    if (!error) {
        ...
        if (!silent) {
            ...
            if (!hideError) {
                ...
                cout.flush();
                int r0 = system(
                        (string() + "powershell /c cat " + currentTrace).c_str());
                (void) r0;
                ...
                if (error == 4) {
                    ...
                    cout.flush();
                    int r1 = system("powershell /c cat test_ans");
                    (void) r1;
                    cout << color("\x1b[0m") << endl;
                    cout << "Your output: " << endl << color("\x1b[33m");
                    cout.flush();
                    int r2 = system("powershell /c cat test_out");
                    (void) r2;
                    cout << color("\x1b[0m") << endl;
                }
            }
